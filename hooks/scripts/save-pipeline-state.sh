...
# Snapshots active pipeline state to .docs/.pipeline-state.json.
...
if [ -d "$CWD/.docs/blueprints" ]; then
  for f in "$CWD/.docs/blueprints"/*.md; do
    [ -f "$f" ] || continue
...
if [ -z "$ACTIVE_SLUG" ] && [ -d "$CWD/.docs/sketches" ]; then
  for f in "$CWD/.docs/sketches"/*.md; do
    [ -f "$f" ] || continue
    if head -15 "$f" | grep -qi "status:.*draft"; then
      ACTIVE_SLUG=$(basename "$f" .md)
...
if [ -z "$ACTIVE_SLUG" ] && [ -d "$CWD/.docs/diagnoses" ]; then
  for f in "$CWD/.docs/diagnoses"/*.md; do
    [ -f "$f" ] || continue
    SLUG=$(basename "$f" .md)
    # If diagnose exists but no sketch, we're between diagnose and sketch
    if [ ! -f "$CWD/.docs/sketches/$SLUG.md" ]; then
      ACTIVE_SLUG="$SLUG"
      ACTIVE_TYPE="fix"
      ACTIVE_PHASE="sketch"
      break
...
if [ -z "$ACTIVE_SLUG" ]; then
  rm -f "$CWD/.docs/.pipeline-state.json"
  exit 0
fi

# Infer phase from artifact presence (if not already set by blueprint check)
if [ "$ACTIVE_PHASE" = "craft" ] && [ -d "$CWD/.docs/retros" ]; then
  # If retro exists for this slug, we're in verify or done
  [ -f "$CWD/.docs/retros/$ACTIVE_SLUG.md" ] && ACTIVE_PHASE="done"
fi

# Write state snapshot
cat > "$CWD/.docs/.pipeline-state.json" << STATEOF
