...
# Reads wiki/ artifact state — no separate memory system needed.
...
if [ -f "$CWD/wiki/.pipeline-state.json" ]; then
  SLUG=$(grep -o '"slug"[[:space:]]*:[[:space:]]*"[^"]*"' "$CWD/wiki/.pipeline-state.json" 2>/dev/null | head -1 | sed 's/.*"slug"[[:space:]]*:[[:space:]]*"\(.*\)"/\1/')
  PHASE=$(grep -o '"phase"[[:space:]]*:[[:space:]]*"[^"]*"' "$CWD/wiki/.pipeline-state.json" 2>/dev/null | head -1 | sed 's/.*"phase"[[:space:]]*:[[:space:]]*"\(.*\)"/\1/')
  TYPE=$(grep -o '"type"[[:space:]]*:[[:space:]]*"[^"]*"' "$CWD/wiki/.pipeline-state.json" 2>/dev/null | head -1 | sed 's/.*"type"[[:space:]]*:[[:space:]]*"\(.*\)"/\1/')
  TIER=$(grep -o '"tier"[[:space:]]*:[[:space:]]*"[^"]*"' "$CWD/wiki/.pipeline-state.json" 2>/dev/null | head -1 | sed 's/.*"tier"[[:space:]]*:[[:space:]]*"\(.*\)"/\1/')
  if [ -n "$SLUG" ]; then
...
for dir in sketches blueprints; do
  ...
  for f in "$CWD/wiki/$dir"/*.md; do
    ...
    if head -15 "$f" | grep -qi "status:.*draft"; then
      SLUG=$(basename "$f" .md)
      LINES="${LINES}DRAFT: wiki/${dir}/${SLUG}.md. "
    fi
  done
done

# 3. Most recent retro (learning continuity)
if [ -d "$CWD/wiki/retros" ]; then
  LATEST=$(ls -t "$CWD/wiki/retros"/*.md 2>/dev/null | head -1)
  if [ -n "$LATEST" ]; then
...
[ -f "$CWD/wiki/MAP.md" ] && LINES="${LINES}MAP.md present — read it for project structure. "
